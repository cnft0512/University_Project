...
public class LectureController {
   ...
   @ResponseBody
   public String listPOST(BasketVO bVo, Model model, Criteria cri, HttpSession session)
         throws Exception {
      ...
      if (!timeList.contains(bVo.getLecture_time())) {
         ...
         if (bList.size() * 3 < 24) {
            ...
            lService.addCount(bVo.getLecture_code(),  bVo.getLecture_year());
   ...
   @ResponseBody
   public String deleteBasketPOST(BasketVO bVo, Model model, Criteria cri, HttpSession session)
         throws Exception {
      log.info("강의 삭제");
      lService.disCount(bVo.getLecture_code(), bVo.getLecture_year());
   ...
   public String timeTableGET(HttpSession session, Model model, HttpServletRequest request) throws Exception {
      ...
      if (request.getParameter("lecture_yearNsemester") != null) {
         ...
         model.addAttribute("sllist", sService.getStudentLectureList(sVo.getId(), Integer.parseInt(session.getAttribute("serverTime").toString().substring(0,4)), "1학기"));
         model.addAttribute("count", sService.getStudentLectureCount(sVo.getId()));
         return "timeTable";
      }
   }
